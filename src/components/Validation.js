...
function LoginCompoent() {
    ...
    const [firstName, setfirstName] = useState({ name: 'firstName', value: '', error: '' });
    const [lastName, setlastName] = useState({ name: 'lastName', value: '', error: '' });
    ...
    const validateForm = () => {
        ...
        if (firstName.value.length === 0 && firstName.value.length <= 2) {
            status = false;
            setfirstName(prevState => { return { ...prevState, error: 'Please enter your first name' } })
        } else {
            setfirstName(prevState => { return { ...prevState, error: '' } })
        }

        if (lastName.value.length === 0 && lastName.value.length <= 2) {
            status = false;
            setlastName(prevState => { return { ...prevState, error: 'Please enter your last name' } })
        } else {
            setlastName(prevState => { return { ...prevState, error: '' } })
        }
